      ...
      if (difp == 1 && difn == 1) {
        if (type == typeLampSig1n) {
          ...
          me = sign * TBME(type, m1, cfgleft[i].lam, m3, cfgright[j].lam);
                   ...
                   typeSig2pSig1n) { // 这里要左右换个位置, 因为 sig1,2,3 有顺序
...
void CreateJsqr(double *eigen_v, double *eigen_v2, struct config *cfgJplus,
  ...
  double *eigen_vJplus = malloc(dimJplus * sizeof(double)); //升算符作用后
...
int shell(int A, int Z, int n_lam, struct level *level, int interaction) {
  ...
  FILE *fp;
  if (!(fp = fopen(path, "rt"))) {
    fprintf(stderr, "Can't find file 1!\n");
    exit(1);
  }
  FILE *fpH;
  ...
  double **h = malloc(sizeof(double *) * dim);
  for (int i = 0; i < dim; i++) {
  ...
  //本征向量
  double complex eigen_values[dim];
  double complex eigen_vec[dim][dim];
  ...
  GetEigenValuesAndVectors(dim, (double *)h1, eigen_values, eigen_vec);
...
*/
  ...
  fprintf(gnuplotPipe, "set output \"result/%s.eps\"\n", name);
  ...
  for (int k = 0; k < kplotmax; k++) {
    if (yn[k + 1] - yn[k] < y[kplotmax] * 0.02) {
      int j;
      for (j = k + 1; j < kplotmax; j++) {
        if (yn[j + 1] - yn[j] >= y[kplotmax] * 0.02) {
          break;
        }
      }
      for (int l = k; l <= j; l++) {
        yn[l] = (y[j] + y[k]) / 2 -
                y[kplotmax] * 0.02 * (((double)(j - k)) / 2 - (double)(l - k));
      }
      k = j;
    }
  }
...
bool shellplot(int A, int Z, int n_lam, int numberoflevels,
  ...
  FILE *fpplot;
  if (!(fpplot = fopen("plot/%s.plt", name))) {
  ...
  shell(A, Z, n_lam, level2, 0);
  double emax = stdmax(level1[numberoflevels - 1].e - level1[0].e,
...
int main() {
  ...
  double val;
  while (ext != 0) {
    printf("1 for shell model, 2 for Λ binding energy, 3 for double-Λ binding "
           "energy, 4 for all p-shell Λ binding energies, 5 for all p-shell ΛΛ "
           "binding energies, 0 exit\n");
    int choose;
    scanf("%d", &choose);
    switch (choose) {
    case 1:
